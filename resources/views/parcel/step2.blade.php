<x-app-layout>
    <div class="container mx-auto flex flex-col items-center">
        <h2 class="text-2xl font-semibold dark:text-white">Sender and Receiver Information</h2>
        @if ($errors->any())
        ...
        <form method="POST" action="{{ route('parcel.storeStep2') }}" class="mt-4">
            ...
            <div class="mt-4">
                ...
                <div class="mb-4">
                    ...
                    <input type="text" id="receiver_name" name="receiver_name" class="form-input dark:text-black" required value="{{ old('receiver_name', $step2Data['receiver_name'] ?? '') }}">
                </div>

                <div class="mb-4">
                    <label for="receiver_email" class="block dark:text-white">Email</label>
                    <input type="email" id="receiver_email" name="receiver_email" class="form-input dark:text-black" required value="{{ old('receiver_email', $step2Data['receiver_email'] ?? '') }}">
                </div>

                <div class="mb-4">
                    <label for="receiver_phone" class="block dark:text-white">Phone</label>
                    <input type="tel" id="receiver_phone" name="receiver_phone" class="form-input dark:text-black" required value="{{ old('receiver_phone', $step2Data['receiver_phone'] ?? '') }}">
                </div>

                <div class="mb-4">
                    <label for="receiver_address" class="block dark:text-white">Address</label>
                    <textarea id="receiver_address" name="receiver_address" class="form-textarea dark:text-black" required placeholder="Enter address">{{ old('receiver_address', $step2Data['receiver_address'] ?? '') }}</textarea>
                </div>
            </div>
