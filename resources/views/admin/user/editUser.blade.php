...
<div class="container mx-auto flex flex-col items-center">
            ...
            <form method="POST" action="{{ route('admin.editUser', $user->id) }}">
                ...
                <div class="grid gap-4 sm:grid-cols-2 sm:gap-6">
                    <div class="sm:col-span-2">
                        ...
                        <input type="text" name="name" id="name" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" value="{{ old('name') ?? $user->name }}" placeholder="Enter name" required autofocus autocomplete="name">
                        @error('name')
                    ...
                    <div class="sm:col-span-2">
                        ...
                        <input type="email" name="email" id="email" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" value="{{ old('email') ?? $user->email }}" placeholder="Enter email" required autocomplete="username">
                        @error('email')
                            <p class="text-red-500 text-sm">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <select id="role" name="role" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" required>
                            ...
                            <option value="1" {{ (old('role') == '1' || $user->role == 1) ? 'selected' : '' }}>Administrator</option>
                            <option value="2" {{ (old('role') == '2' || $user->role == 2) ? 'selected' : '' }}>Courier</option>
                            <option value="0" {{ (old('role') == '0' || $user->role == 0) ? 'selected' : '' }}>Registered User</option>
                        </select>
                        @error('role')
                        <p class="text-red-500 text-sm">{{ $message }}</p>
                        @enderror
    ...
    <section class="bg-white dark:bg-gray-900">
        <div class="py-8 px-4 mx-auto max-w-2xl lg:py-16 w-full">
            <h2 class="mb-4 text-xl font-bold text-gray-900 dark:text-white">Edit Password</h2>
            <form method="POST" action="{{ route('admin.editUserPassword', $user->id) }}">
                @csrf
                @method('PUT')
                <div class="grid gap-4 sm:grid-cols-2 sm:gap-6">
                    <div class="sm:col-span-2">
